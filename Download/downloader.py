...
class Downloader():
    ...
    def crowl(self):
        ...
        if  len(sub) - number_of_images > 0 :
            self.sub = sub[:number_of_images - len(sub)]

        self.download()

    def download(self):
        try:
            os.mkdir(self.config['download_folder'])
        ...
        for i in tqdm(range(len(sub))):
            ...
            try:
                if src != None:
                    src  = str(src)
                    count+=1
                    urllib.request.urlretrieve(src, os.path.join(self.config['download_folder'],'image'+str(count)+'.jpg'))
                else:
                    raise TypeError
            except TypeError:
                error += 1
